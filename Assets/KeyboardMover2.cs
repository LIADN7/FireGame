...
public class KeyboardMover2 : MonoBehaviour
{
    ...
    void Update()
    {
        float x = Input.GetAxis("Horizontal1"); // +1 if right arrow is pushed, -1 if left arrow is pushed, 0 otherwise
        float y = Input.GetAxis("Vertical1");     // +1 if up arrow is pushed, -1 if down arrow is pushed, 0 otherwise
